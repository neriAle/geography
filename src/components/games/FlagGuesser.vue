...
onMounted(() => {
  ...
  // 2. Filter valid countries (Must have ISO and a Name)
...
// Pass this to the ComboBox so users can only search valid answers
const allValidAnswers = computed(() => {
  return (countriesData as CountryData[]).filter((c) => c.iso && c.name);
});
...
<template>
  <div class="mx-auto mt-8 flex w-full max-w-2xl flex-col items-center p-4">
    ...
    >
      ...
      >
        <h2
          class="mb-6 text-sm font-bold tracking-widest text-slate-400 uppercase"
        >
        ...
        <img
          ...
          alt="Mystery Flag"
          class="mb-2 h-auto w-64 rounded border border-slate-100 object-cover shadow-sm"
        />
      </div>
      ...
      <div class="relative h-32 w-full">
        ...
        >
          ...
          >
            ...
            >
              You chose
              <span class="font-bold text-slate-800">{{ userGuess?.name }}</span
              >.
            </p>
